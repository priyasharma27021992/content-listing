...
const CardComponent = ({ data }) => {
  ...
  return (
    <div className='flex min-h-100 min-w-50 flex-col gap-4 rounded-md shadow-md'>
      <div className='flex flex-col gap-2'>
        {image && (
          <Image src={image.src} alt={image.alt} height={50} width={200} />
        )}
        <h2>{title}</h2>
        <h3>{subTitle}</h3>
      </div>
...
const HotDeals = () => {
  ...
  return (
    <section>
      ...
      <CardCarousel cards={deals} CardComponent={CardComponent} />
    </section>
  );
};
